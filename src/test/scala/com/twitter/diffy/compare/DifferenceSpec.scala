  ...
  describe("Difference") {
    it("should detect when there is no difference between two objects") {
      ...
      testCases foreach { case (left, right) =>
        Difference(left, right, DifferenceConf.apply(0.0,"", false)) must be(NoDifference(Difference.lift(left)))
        Difference(JsonLifter(left), JsonLifter(right), DifferenceConf.apply(0.0,"", false)) must be(NoDifference(Difference.lift(JsonLifter(left))))
      }
    }
    ...
    it("should detect when two primitives are not equal") {
      ...
      testCases foreach { case (left, right) =>
        Difference(left, right, DifferenceConf.apply(0.0,"", false)) must be(PrimitiveDifference(left,right))
      }
    }
    ...
    it("should throw TypeDifference when 2 primitives are of different types") {
      ...
      testCases foreach { case (left, right) =>
        Difference(left, right, DifferenceConf.apply(0.0,"", false)) must be(TypeDifference(left, right))
      }
    }
    ...
    it("should detect a difference between JsonNull and other objects") {
      ...
      testCases foreach { testCase =>
        Difference(JsonLifter.JsonNull, testCase, DifferenceConf.apply(0.0,"", false)) must
          be(TypeDifference(JsonLifter.JsonNull, testCase))
      }
    }

    it("should detect no difference when two JsonNull objects are compared") {
      Difference(JsonLifter.JsonNull, JsonLifter.JsonNull, DifferenceConf.apply(0.0,"", false)) must be(NoDifference(JsonLifter.JsonNull))
    }

    it("should return an OrderingDifference when two Seqs have the same elements but different order") {
      Difference(Seq("a", "b", "c"), Seq("c", "b", "a"), DifferenceConf.apply(0.0,"", false)) must be(
    ...
    it("should return a SeqSizeDifference when two Seqs are of different size") {
      Difference(Seq("a", "a", "b", "b"), Seq("a", "b", "b", "c", "c"), DifferenceConf.apply(0.0,"", false)) must be(
        SeqSizeDifference(
          leftNotRight = Seq("a"),
    ...
    it("should return a IndexedDifference when two Seqs are of the same size but have different elements") {
      Difference(Seq("a", "b", "c"), Seq("a", "b", "d"), DifferenceConf.apply(0.0,"", false)) must be(
        IndexedDifference(
          indexedDiffs =
            Seq(
              NoDifference("a"),
    ...
    it("should detect when two Sets are not equal") {
      Difference(Set(1, 2, 3), Set(2, 3, 4), DifferenceConf.apply(0.0,"", false)) must be(
        SetDifference(leftNotRight = Set(1), rightNotLeft = Set(4))
      )
    }

    it("should detect when two Maps are not equal") {
      Difference(Map(1 -> 2, 2 -> 3, 3 -> 4), Map(1 -> 2, 2 -> 4, 4 -> 5), DifferenceConf.apply(0.0,"", false)) must be(
        MapDifference(
    ...
    it("should detect when two case class instances are not equal") {
      Difference(Name("puneet", "khanduri"), Name("prashant", "khanduri"), DifferenceConf.apply(0.0,"", false)) must be(
    ...
    it("should detect difference between binaries embedded within thrift structs") {
      ...
      Difference(left,right, DifferenceConf.apply(0.0,"", false)) must be (
